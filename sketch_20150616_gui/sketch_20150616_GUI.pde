...
int anzahl=50;                                                // interger for fading section cut
...
void draw() {
 ...
 float stepperMap = map (stepperCount, 0, 500, 380, 1715);                      // remap stepperCount to section window
  
  float ypos = 1005 - (bannerValue/4);                                          // draw line
  stroke(255);
  line(stepperMap, 1005, stepperMap, height - (bannerValue/4));
